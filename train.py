...
G, C1, C2, opt_g, opt_c, \
param_lr_g, param_lr_c, scaler = get_models(inputs)
ndata = target_folder.__len__()


def train():
    sampler = None
    extension = extensions.get_method('generate')
    ...
    for step in range(conf.train.min_step + 1):
        if step == 5000 and extension is not None and sampler is None:
            sampler = extension(G, [C1, C2], target_loader,
        ...
        if step >= 5000 and extension is not None and sampler is not None:
            img_t_unk = sampler()
            img_t_unk = Variable(img_t_unk.cuda())
            feat_t_unk = G(img_t_unk)
            out_open_t_unk = C2(feat_t_unk)
            out_open_t_unk = out_open_t_unk.view(out_open_t_unk.size(0), 2, -1)
            open_loss_t_unk = extensions.ova_loss_unk(out_open_t_unk)
            all += 0.05 * open_loss_t_unk
            log_values.append(open_loss_t_unk.item())
            log_string += 'Loss Open Target Unknown: {:.6f} '
